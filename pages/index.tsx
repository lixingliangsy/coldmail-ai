...
import { STEP_ORDER, STEP_LABELS, StepId } from '../lib/pipeline'
import { RULESET_VERSION } from '../lib/rules/cold-outreach'
...
/** Multi-step workflow studio (L1): drives the pipeline, shows step list + demo banner + rule checks + export/history. */
function WorkflowStudio() {
  ...
  const setVal = (k: string, v: string) => setInputs((s) => ({ ...s, [k]: v }))
  ...
  async function callTool(body: any) {
    const payload = {
      ...body,
      useMock: !!(body.useMock ?? body.useDemoMode),
    }
    delete (payload as any).useDemoMode
  ...
  async function start() {
    ...
    setRunStatus('')
    try {
      const { r, d } = await callTool({ useDemoMode, inputs, action: 'start' })
      if (!r.ok) {
      ...
      setResult(d.result || '')
      setDemo(!!d.demo)
      setRuleResults(d.ruleHits || d.ruleResults || [])
    } catch (e: any) {
  ...
  async function advance() {
    ...
    setError('')
    try {
      const { r, d } = await callTool({ useDemoMode, runId, step: currentStep, action: 'advance' })
      if (!r.ok) {
        setError(d.error || 'Request failed')
        return
      }
  ...
  async function confirm() {
    ...
    setError('')
    try {
      const { r, d } = await callTool({ useDemoMode, runId, step: currentStep, action: 'confirm' })
      if (!r.ok) {
        setError(d.error || 'Request failed')
        return
      }
      setCurrentStep(d.step)
      setRunStatus(d.status || 'done')
      setResult(d.result || '')
      setDemo(!!d.demo)
      setRuleResults(d.ruleHits || d.ruleResults || [])
      showToastMsg('Workflow finalized')
    } catch (e: any) {
      setError(e?.message || 'Error')
    } finally {
      setLoading(false)
  ...
  async function loadHistory() {
    try {
      ...
      const d = await r.json()
      if (r.ok && d.ok) setHistory(d.runs || [])
    } catch {
      /* optional */
    }
    setShowHistory(true)
  ...
  function exportRuns() {
    fetch('/api/runs')
      .then((r) => r.json())
      .then((d) => {
        const blob = new Blob([JSON.stringify(d.runs || [], null, 2)], { type: 'application/json' })
        const url = URL.createObjectURL(blob)
        const a = document.createElement('a')
        a.href = url
        a.download = `${PRODUCT.slug}-runs.json`
        a.click()
  ...
  const nextLabel = currentIdx < steps.length - 1 ? STEP_LABELS[steps[currentIdx + 1]] : ''

  return (
    <div className="rounded-2xl border border-slate-200 p-6 bg-white shadow-sm">
      <div className="flex flex-wrap items-center justify-between mb-4 gap-3">
        <h3 className="font-bold">Workflow studio</h3>
        <div className="flex gap-3">
          <button type="button" onClick={loadHistory} className="text-sm text-indigo-600 font-semibold">Run history</button>
          <button type="button" onClick={exportRuns} className="text-sm text-indigo-600 font-semibold">Export runs (JSON)</button>
        </div>
      </div>
        ...
        <div key={f.key} className="mb-4">
          ...
          {f.type === 'textarea' ? (
            <textarea
              ...
              className="w-full h-24 p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand focus:outline-none"
            />
          ) : f.type === 'select' ? (
            <select
              value={inputs[f.key] || ''}
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        {!runId ? (
          <button onClick={start} disabled={loading} className="px-5 py-3 rounded-full bg-indigo-600 text-white font-bold disabled:opacity-60">
            {loading ? 'Running…' : 'Start workflow'}
          </button>
        ) : !done ? (
          currentStep === 'output' ? (
            <button onClick={confirm} disabled={loading} className="px-5 py-3 rounded-full bg-emerald-600 text-white font-bold disabled:opacity-60">
              {loading ? 'Finalizing…' : 'Confirm & finalize'}
            </button>
          ) : (
            <button onClick={advance} disabled={loading} className="px-5 py-3 rounded-full bg-indigo-600 text-white font-bold disabled:opacity-60">
              {loading ? 'Running…' : `Run step ${currentIdx + 2}: ${nextLabel}`}
            </button>
          )
        ) : (
          <span className="text-sm font-semibold text-emerald-700">✓ Workflow complete</span>
        )}
        <label className="flex items-center text-sm text-gray-600 ml-auto">
          <input type="checkbox" checked={useDemoMode} onChange={(e) => setUseDemoMode(e.target.checked)} className="mr-2" />
      ...
      {demo ? (
        <div className="mb-4 text-xs font-semibold text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
          Demo mode — not live AI. Add an API key in .env.local for real generation.
        </div>
      ) : null}
        ...
        <div className="bg-gray-50 border border-slate-200 rounded-xl p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-bold text-slate-700">
              Step {currentIdx + 1}/{steps.length} · {STEP_LABELS[currentStep]} output
            </span>
            <button type="button" onClick={() => { navigator.clipboard?.writeText(result); showToastMsg('Copied') }} className="text-sm text-indigo-600 font-semibold">
              Copy
            </button>
          </div>
          <pre className="whitespace-pre-wrap text-sm text-slate-700 max-h-72 overflow-auto">{result}</pre>
        </div>
      ) : null}

      {ruleResults && ruleResults.length ? (
        <div className="mt-4">
          <div className="text-xs font-bold text-slate-500 mb-2">Ruleset {RULESET_VERSION} · checks</div>
          <ul className="space-y-1">
            {ruleResults.map((rr: any) => (
              <li key={rr.ruleId} className="text-xs flex gap-2">
                <span className={rr.passed ? 'text-emerald-600' : 'text-rose-600'}>{rr.passed ? '✓' : '✗'}</span>
                <span className="text-slate-600">{rr.name} — {rr.message}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      {showHistory ? (
        <div className="mt-6 border-t border-slate-200 pt-4">
          <div className="text-sm font-bold mb-2">Run history</div>
          {history.length === 0 ? (
            <div className="text-sm text-slate-500">No runs yet — start a workflow above.</div>
          ) : (
            <ul className="text-xs space-y-1 max-h-48 overflow-auto">
              {history.slice(0, 20).map((h: any) => (
                <li key={h.runId} className="text-slate-600">
                  {(h.createdAt || '').slice(0, 19).replace('T', ' ')} · step {h.step} · {h.status}
                </li>
              ))}
            </ul>
          )}
  ...
  return (
    <>
      <Head>
        ...
        <link rel="canonical" href="https://coldmail-ai.lxsaihub.com/" />
      
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: `{"@context": "https://schema.org", "@type": "FAQPage", "mainEntity": [{"@type": "Question", "name": "What is ColdMail AI?", "acceptedAnswer": {"@type": "Answer", "text": "A tool that generates research-backed, personalized cold emails for outbound."}}, {"@type": "Question", "name": "How is it personalized?", "acceptedAnswer": {"@type": "Answer", "text": "It uses prospect context instead of a generic blast template."}}, {"@type": "Question", "name": "What tones are available?", "acceptedAnswer": {"@type": "Answer", "text": "Founder, formal, and casual tones, with subject-line variants."}}, {"@type": "Question", "name": "Does it help with follow-ups?", "acceptedAnswer": {"@type": "Answer", "text": "Yes — reply and follow-up generators keep the thread consistent."}}, {"@type": "Question", "name": "Which channels does it cover?", "acceptedAnswer": {"@type": "Answer", "text": "LinkedIn, X, and email with channel-appropriate phrasing."}}, {"@type": "Question", "name": "Who should use it?", "acceptedAnswer": {"@type": "Answer", "text": "Founders and freelancers who want reply-worthy emails without writing every variant by hand."}}]}` }} />
        </Head>
      ...
      <div className="min-h-screen bg-white text-slate-900">
        ...
        <section id="definition" className="py-20 bg-slate-50">
          <div className="max-w-6xl mx-auto px-6">
            ...
            <h2 className="text-3xl md:text-4xl font-extrabold mb-4">ColdMail AI - definition</h2>
...
<section id="features" className="py-20">
        ...
        <section id="studio" className="py-20">
          <div className="max-w-6xl mx-auto px-6">
            <div className="text-center mb-12">
              <div className="text-xs font-bold tracking-widest uppercase text-indigo-600 mb-3">Workflow studio</div>
              <h2 className="text-3xl md:text-4xl font-extrabold">Run the workflow on this page</h2>
              <p className="text-slate-600 mt-2">
                <button type="button" className="text-indigo-600 font-bold underline" onClick={openDemo}>Watch demo</button>
                {' '}or run the 4-step workflow below.
              </p>
            </div>
            <WorkflowStudio />
            <div className="text-center mt-6">
              <a href="/api/checkout" onClick={(e) => goCheckout('/api/checkout', e)} className="px-4 py-2 rounded-full bg-indigo-600 text-white font-semibold">
                ${price}/mo — Upgrade for more runs
              </a>
            </div>
          </div>
        </section>
